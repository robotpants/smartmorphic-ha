...
- [x] **Scene chip** — pill that recedes (pressed inset) when its scene is the most-recently-activated. Lives in a horizontal scroll.
...
- [x] **Tile card** (`TileCard`) — generic raised entity tile (light/switch/binary_sensor/cover/script/input_*). ActiveIconWell + label/sub + state stat.
- [x] **Climate zone card** (`ClimateTile`) — round target dial, Heat/Cool/Auto/Off pill row, current temp + setpoint delta, humidity row.
- [x] **Sensor tile** (`SensorTile`) — eyebrow + label + Stat (32px display) + 140×28 sparkline + optional delta footer.
- [x] **Automation row card** — trigger-type chip + name, description, last-triggered timestamp, Run button, enable toggle.
